...
class Conversation:
    ...
    # デフォルトプロンプト上書きフラグ(True:上書き済み,False:上書きなし)
    __override_default_system_prompt = False
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for role, message in self.messages:
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style in [SeparatorStyle.TSUZUMI_HF]:
            seps = [self.sep, self.sep2]
            system_prompt = self.system_template.format(system_message=self.system_message) if self.system_message else ""
            ret = ""
            for i,(role, message) in enumerate(self.messages):
                # 1要素目かつ、roleが"user"でない、かつsystem_promptが指定されている場合
                # system_promptを先頭に挿入
                if (i == 0) and ("user" not in role) and (system_prompt != ""):
                    ret = f"<|user|>{system_prompt}{seps[0]}"
                if message:
                    if "user" in role:
                        if len(ret):
                            ret += role + message + seps[0]
                        else:
                            ret += role + system_prompt + message + seps[0]
                    else:
                        ret += message + seps[1]
                else:
                    ret += ""
    ...
    def set_system_message(self, system_message: str):
        ...
        if self.sep_style in [SeparatorStyle.TSUZUMI_HF]:
            if self.__override_default_system_prompt:
                # 2要素目以降は\n区切りで追加
                self.system_message += "\n" + system_message
            else:
                # 1要素目はデフォルトプロンプトを上書き
                self.system_message = system_message
                self.__override_default_system_prompt = True
        else:
            self.system_message = system_message
...
register_conv_template(
    Conversation(
        name="tsuzumi-hf",
        system_message=os.getenv("WORKER_SYSTEM_PROMPT", ""),
        system_template="{system_message}\n\n",
        roles=("<|user|>", "<|assistant|>"),
        sep_style=SeparatorStyle.TSUZUMI_HF,
        sep="<|endofuser|>",
        sep2="<|endofassistant|>",
        stop_str="<|endoftext|>"
    )
