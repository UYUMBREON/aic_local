...
from fastchat.model.model_tsuzumi_hf import generate_stream_tsuzumi_hf
...
from fastchat.constants import (
    LOG_LEVEL_MODEL_ADAPTER,
)
from fastchat.utils import build_logger
logger = build_logger("model_adapter", "model_adapter.log", LOG_LEVEL_MODEL_ADAPTER)
...
def load_model(
    ...
    # Load model
    model, tokenizer = adapter.load_model(model_path, attn_impl, kwargs)
...
def get_generate_stream_function(model: torch.nn.Module, model_path: str):
    ...
    is_tsuzumi_hf = os.getenv('WORKER_MODEL_TYPE') == "tsuzumi-hf"

    if is_tsuzumi_hf:
        return generate_stream_tsuzumi_hf
    elif is_chatglm:
...
class TsuzumiHFAdapter(BaseModelAdapter):

    def match(self, model_path: str):
        if os.getenv('WORKER_MODEL_TYPE') == "tsuzumi-hf":
            return True
        return False

    def load_model(self, model_path: str, attn_impl: str, from_pretrained_kwargs: dict):
        config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)

        # TsuzumiHF版Fastchat改造箇所
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        return get_conv_template("tsuzumi-hf")
...
register_model_adapter(TsuzumiHFAdapter)
