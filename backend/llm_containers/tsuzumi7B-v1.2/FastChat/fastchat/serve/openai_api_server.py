...
import uuid
import re
from typing import Generator, Optional, Union, Dict, List, Any, Callable
...
from fastchat.constants import (
    ...
    ErrorCode,
    ENABLE_INOUT_LOG,
    LOG_LEVEL_OPEN_API_SERVER,
    LOG_MESSAGE_BAD_REQUEST,
    LOG_MESSAGE_UNAUTHORIZED,
    LOG_MESSAGE_INTERNAL_SERVER_ERROR,
    LOG_MESSAGE_OTHER_ERROR,
    SERVER_ERROR_MSG
)
...
# アプリ用ログ
app_logger = build_logger("open_api_server", "open_api_server.log", LOG_LEVEL_OPEN_API_SERVER)
...
class LoggingRoute(APIRoute):
    """ロガールータクラス
    オリジナルのルータ処理をフックし、リクエスト処理前後にログ出力処理を挿入
    """
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            ...
            # リクエストURL
            request_info["request_url"] = request.url.path
            # HTTPメソッド
            request_info["request_method"] = request.method
            # リクエストボディ
            if await request.body() and ENABLE_INOUT_LOG:
                try:
                    request_info["request_body"] = json.loads((await request.body()).decode("utf-8"))
                except json.JSONDecodeError as e:
                    # 入力がJSON形式不正の場合は格納不可
                    pass
            # リクエストヘッダ
            request_info["request_headers"] = {
                key.decode("utf-8"): value.decode("utf-8") for (key, value) in request.headers.raw
            }
            # 接続元ホスト
            request_info["request_host"] = request.client.host
            app_logger.info("request_info: " + json.dumps(request_info, ensure_ascii=False))
            ...
            if type(response) != StreamingResponse:
                if response.status_code == 200:
                    ...
                    app_logger.info("response_info: " + json.dumps(response_info, ensure_ascii=False))
                else:
                    error_message = json.loads(response.body.decode("utf-8"))["message"]
                    if SERVER_ERROR_MSG in error_message:
                       error_message = error_message.replace(SERVER_ERROR_MSG + "\n\n", "")[1:-1]
                    error_response_log(request_id, status_code=response.status_code, exception=error_message)

            return response

        return custom_route_handler

async def logging_chat_completion_stream_generator(
        request_id: str,
    ...
    response_info["response_body"] = None
    async for response in generator:
        response_body = response_info["response_body"]
        if response == "data: [DONE]\n\n":
            if not ENABLE_INOUT_LOG:
                response_info.pop("response_body")
            app_logger.info("response_info: " + json.dumps(response_info, ensure_ascii=False))
        else:
            json_data = json.loads(response.strip().replace('data: ', '', 1))

            # エラー応答時
            if "error_code" in json_data:
                error_code = int(json_data["error_code"])
                error_message = json_data["text"]
                if SERVER_ERROR_MSG in error_message:
                    error_message = error_message.replace(SERVER_ERROR_MSG + "\n\n", "")[1:-1]
                if error_code > 40000 and error_code < 50000:
                    error_response_log(request_id, status_code=400, exception=error_message, stream=True)
                elif error_code > 50000:
                    error_response_log(request_id, status_code=500, exception=error_message, stream=True)
                yield response
                continue
            ...
            if response_body is None:
                ...
                if "created" in json_data:
                    response_body["created"] = json_data["created"]
                if "object" in json_data:
                    response_body["object"] = json_data["object"]
                choices = response_body["choices"]
                if "choices" in json_data:
                    for value in json_data["choices"]:
                        i = value["index"]
                        if len(choices) > i:
                            if ("finish_reason" in value):
                                choices[i]["finish_reason"] = value["finish_reason"]
                            if ("role" not in choices[i]["delta"]) and ("role" in value["delta"]):
                                choices[i]["delta"]["role"] = value["delta"]["role"]
                            if ("content" not in choices[i]["delta"]) and ("content" in value["delta"]):
                                choices[i]["delta"]["content"] = value["delta"]["content"]
                            elif "content" in value["delta"]:
                                choices[i]["delta"]["content"] += value["delta"]["content"]
                        else:
...
def error_response_log(
    ...
    """
    Args:
        request_id (str): リクエストID(必須)
        status_code (int): ステータスコード
       exception (Exception): 例外情報
        stream (bool): ストリームON/OFF
    """
    error_info = {}
    ...
    # レスポンスのステーたコードを追加(ストリーム時は200で帰る)
    if not stream:
        error_info["status_code"] = status_code
    else:
        error_info["status_code"] = 200
    ...
    if status_code == 400:
        app_logger.warning(f"{LOG_MESSAGE_BAD_REQUEST}: " + json.dumps(error_info, ensure_ascii=False))
    elif status_code == 401:
        app_logger.warning(f"{LOG_MESSAGE_UNAUTHORIZED}: " + json.dumps(error_info, ensure_ascii=False))
    elif status_code == 500:
        app_logger.error(f"{LOG_MESSAGE_INTERNAL_SERVER_ERROR}: " + json.dumps(error_info, ensure_ascii=False))
    else:
        app_logger.error(f"{LOG_MESSAGE_OTHER_ERROR}: " + json.dumps(error_info, ensure_ascii=False))
...
async def check_api_key(
    ...
    if app_settings.api_keys:
        if auth is None or (token := auth.credentials) not in app_settings.api_keys:
            error_response_log(x_request_id, status_code=401)
            raise HTTPException(
                status_code=401,
                detail={
                    "error": {
...
async def validation_exception_handler(request, exc):
    ...
    error_response_log(request_id, status_code=400, exception=exc)
    ...
    if isinstance(inp, str):
        ...
        if isinstance(inp[0], int):
            try:
                ...
                app_logger.warning("Warning: model not found. Using cl100k_base encoding.")
                model = "cl100k_base"
...
async def create_chat_completion(request: ChatCompletionRequest, x_request_id: str = Header()):
    ...
    # パラメータを追加
    gen_params["min_new_tokens"] = request.min_tokens
    gen_params["top_k"] = request.top_k
    gen_params["repetition_penalty"] = request.repetition_penalty
    gen_params["no_repeat_ngram_size"] = request.no_repeat_ngram_size
    gen_params["do_sample"] = request.do_sample
    gen_params["use_cache"] = request.use_cache
    gen_params["early_stopping"] = request.early_stopping

    if request.stream:
        generator = chat_completion_stream_generator(
            request.model, gen_params, request.n, worker_addr
        )
        logging_generator = logging_chat_completion_stream_generator(x_request_id, generator)
...
def create_openai_api_server():
    ...
    # 空文字列指定の場合は認証無しとする
    if (args.api_keys is None) or ((len(args.api_keys) == 1) and (args.api_keys[0] == "")):
        app_settings.api_keys = None
    else:
        app_settings.api_keys = args.api_keys

    app_logger.info(f"args: {args}")
    return args
